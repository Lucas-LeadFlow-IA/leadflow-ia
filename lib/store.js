...
// Supabase integration optional - uncomment and install @supabase/supabase-js when ready
...
export const useStore = create(
  persist(
    (set, get) => ({
      ...
      history: [],
      addToHistory: (entry) => {
        const local = { ...entry, id: Date.now(), timestamp: new Date().toISOString() }
        const { history } = get()
        set({
          history: [local, ...history.slice(0, 99)]
        })
        // To sync with Supabase: install @supabase/supabase-js and uncomment
        // if (user && supabase) { supabase.from('history').insert({...}) }
      },
      ...
      contacts: [],
      addContact: (contact) => {
        const local = { ...contact, id: Date.now(), createdAt: new Date().toISOString() }
        const { contacts } = get()
        set({ contacts: [local, ...contacts] })
        // To sync with Supabase: install @supabase/supabase-js and uncomment
      },
      ...
      saveResult: (moduleId, result, name) => {
        const local = { id: Date.now(), moduleId, result, name, createdAt: new Date().toISOString() }
        const { savedResults } = get()
        set({ savedResults: [local, ...savedResults] })
        // To sync with Supabase: install @supabase/supabase-js and uncomment
      },
      ...
      // Load data from Supabase (call after login) - optional feature
      // To enable: install @supabase/supabase-js and uncomment in supabase.js
      loadUserData: async () => {
