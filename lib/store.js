...
const createClientStore = () => create(persist((set, get) => ({
  trialLeads: [],
  addTrialLead: (email, name, company, password) => {
    const { trialLeads, user } = get()
    if (!trialLeads.find(l => l.email === email)) {
      set({ trialLeads: [...trialLeads, { id: Date.now(), email, name, company, password, ownerEmail: user?.email, createdAt: new Date().toISOString() }] })
    }
  },
  ...
  login: async (email, password) => {
    ...
    const storedUser = get().trialLeads.find(l => l.email === email && l.password === password)
    if (storedUser) {
      const user = { id: storedUser.id, email: storedUser.email, name: storedUser.name, plan: 'free', requestsUsed: 0 }
      set({ user })
      return { success: true }
    }
  ...
  register: async (email, password, name) => {
    await new Promise(resolve => setTimeout(resolve, 500))
    const { addTrialLead } = get()
    addTrialLead(email, name, '', password)
    set({ user: { id: String(Date.now()), email, name, plan: 'free', requestsUsed: 0 } })
    return { success: true }
  },
  ...
  history: [],
  addToHistory: (item) => set((state) => ({ history: [{ ...item, ownerEmail: state.user?.email }, ...state.history.slice(0, 49)] })),
  clearHistory: () => set({ history: [] }),
  savedResults: [],
  saveResult: (moduleId, input, output) => set((state) => ({
    savedResults: [...state.savedResults, { ownerEmail: state.user?.email, id: Date.now(), moduleId, input, output, createdAt: new Date().toISOString() }]
  })),
  deleteSavedResult: (id) => set((state) => ({ savedResults: state.savedResults.filter(r => r.id !== id) })),
  contacts: [],
  addContact: (contact) => set((state) => ({ contacts: [...state.contacts, { ...contact, ownerEmail: state.user?.email, id: Date.now(), createdAt: new Date().toISOString() }] })),
